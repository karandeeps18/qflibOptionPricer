                          ...
                          double correl)
{
  ...
  // Modified divindend yield q' = rd - rf + q - rho * sigma * sigma_f 
  double modDiv = discRate - growthRate + divYield - correl * assetVol * fxVol;

  // use modDiv as the dividend yield in the Black-Scholes model
  double fwd = fwdPrice(spot, timeToExp, discRate, modDiv);
  ...
  // Price and Greeks
  double price = phi * df * (fwd * nd1 - strike * nd2);
  double delta = phi * exp(-modDiv * timeToExp) * nd1;
  double gamma = exp(-modDiv * timeToExp) * nprd1 / (spot * assetVol * sqrtT);
  double theta = -exp(-modDiv * timeToExp) * nprd1 * spot * assetVol / (2.0 * sqrtT) 
      + phi * modDiv * exp(-modDiv * timeToExp) * spot * nd1
      - phi * discRate * df * strike * nd2;
  double vega = exp(-modDiv * timeToExp) * sqrtT * nprd1;
